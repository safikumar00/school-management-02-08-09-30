...
export function AttendanceList({ data, loading = false }: AttendanceListProps) {
  ...
  return (
    <View style={styles.container}>
      {data.map((item) => (
        <Card key={`${item.subject}-${item.lastUpdated}`} style={styles.item}>
...
const styles = StyleSheet.create({
  ...
  classCount: { fontSize: TYPOGRAPHY.fontSize.sm, color: COLORS.gray[600] },
  lastUpdated: { fontSize: TYPOGRAPHY.fontSize.xs, color: COLORS.gray[500] },
  progressBar: {
    height: 6,
